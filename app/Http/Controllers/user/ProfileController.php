...
class ProfileController extends Controller
{
    public function index()
    {
        ...
        $memberships = ProfessionalMembership::where('user_id', Auth::id())->get();
        $employmentHistories = EmploymentHistory::where('user_id', Auth::id())->get();
        $referees = Referee::where('user_id', Auth::id())->get();

        return view('user.profile', compact([ 
            'user','records','qualifications','memberships','employmentHistories','referees'
        ]));
    }   
    ...
    public function updatePersonalDetails(Request $request)
    {
        ...
        $user->update($request->only('name', 'title', 'phone', 'id_passport', 'kra_pin',
        'county', 'sub_county', 'ethnicity', 'nationality', 'gender', 'dob','disability_status','disability_certificate_number'));
    ...
    {
        // Logic to update education background
        // Validate and update education data
        $request->validate([
    ...
    {
        // Logic to update professional qualifications
        // Validate and update qualifications data
        $request->validate([
    ...
    {
        // Logic to update professional memberships
        // Validate and update memberships data
        $request->validate([
            'description' => 'required|string',
            'file' => 'required|file|mimes:pdf|max:2048', // Only allow PDF files
        ]);
    ...
    {
        // Logic to update employment history
        // Validate and update employment data
        $request->validate([
    ...
    public function uploadDocuments(Request $request)
    {
        ...
        if ($request->hasFile('cover_letter')) {
            $path = $request->file('cover_letter')->store('uploads/cover_letters');
    ...
    {
        // Logic to update references
        // Validate and update references data
        $request->validate([
    ...
    public function myApplications(Request $request){
        $applications = Application::with(['jobListing', 'user']) // Eager load related models
