...
@section('content')
    <div class="container">
        ...
        <div class="card mb-4">
            <div class="card-body">
                <form action="{{ route('academic_records.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="mb-3">
                        ...
                        <select class="form-control @error('qualification_code') is-invalid @enderror" name="qualification_code" required>
                            ...
                            <option value="1" {{ old('qualification_code') == 'Certificate' ? 'selected' : '' }}>Certificate</option>
                            <option value="2" {{ old('qualification_code') == 'Diploma' ? 'selected' : '' }}>Diploma</option>
                            <option value="3" {{ old('qualification_code') == 'Degree' ? 'selected' : '' }}>Degree</option>
                            <option value="4" {{ old('qualification_code') == 'Master' ? 'selected' : '' }}>Master</option>
                            <option value="5" {{ old('qualification_code') == 'PhD' ? 'selected' : '' }}>PhD</option>
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('qualification_cadre') is-invalid @enderror" 
                               name="qualification_cadre" value="{{ old('qualification_cadre') }}" required>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="date" class="form-control @error('graduation_date') is-invalid @enderror" 
                               name="graduation_date" value="{{ old('graduation_date') }}" required>
